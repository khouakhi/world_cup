...
import { FIRESTORE_QUOTA_MESSAGE, isFirestoreQuotaError } from "@/lib/db/errors";
import { getMainLeagueInviteCode, isLeagueAdmin } from "@/lib/constants";
import { z } from "zod";
...
function quotaResponse() {
  return NextResponse.json(
    { error: FIRESTORE_QUOTA_MESSAGE, code: "QUOTA_EXCEEDED" },
    { status: 503 }
  );
}
...
export async function GET(request: NextRequest) {
  ...
  const mainOnly = request.nextUrl.searchParams.get("main") === "1";
